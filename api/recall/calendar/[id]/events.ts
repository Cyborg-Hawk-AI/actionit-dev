...
export async function GET(
  ...
  try {
    console.log('[Recall.ai API] ===== GET CALENDAR EVENTS REQUEST =====');
    ...
    // Build URL for Recall.ai API
    let url = `${RECALL_BASE}/api/v2/calendar-events/?calendar_id=${params.id}`;
    if (updatedSince) {
    ...
    if (!recallResponse.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to get calendar events: ${errorData.detail || errorData.error || recallResponse.statusText}` },
        { status: recallResponse.status }
      );
    }

    const events = await recallResponse.json();
    console.log('[Recall.ai API] Calendar events retrieved successfully:', events.results?.length || 0, 'events');
    
    return NextResponse.json(events);

  } catch (error) {
    console.error('[Recall.ai API] Failed to get calendar events:', error);
    return NextResponse.json(
      { error: 'Failed to get calendar events', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
