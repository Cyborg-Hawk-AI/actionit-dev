...
export default async function handler(req, res) {
  try {
    console.log('[Recall.ai API] ===== CREATE CALENDAR REQUEST (JS) =====');
    ...
    try {
      ...
      return res.status(500).json({
        error: `Failed to make request to Recall.ai: ${fetchError.message}`
      });
    }
    ...
    console.error('[Recall.ai API] Error name:', error.name);
    console.error('[Recall.ai API] Error message:', error.message);
    console.error('[Recall.ai API] Error stack:', error.stack);

    return res.status(500).json({
      error: 'Failed to create Recall.ai calendar',
