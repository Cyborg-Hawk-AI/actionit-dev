...
serve(async (req) => {
  ...
  try {
    ...
    if (event === "bot.status_change") {
      ...
      return new Response(JSON.stringify({ status: "received", event }), {
...
async function handleBotStatusChange(data) {
  ...
  if (["call_ended", "done", "fatal"].includes(status.code)) {
    ...
    try {
      ...
      if (!recordingData) {
        ...
        return new Response(
          JSON.stringify({ error: "No recording found for this bot" }),
      ...
      if (!analyzeResponse.ok) {
        ...
        return new Response(
          JSON.stringify({ error: "Failed to analyze bot", details: errorText }),
      ...
      return new Response(
        JSON.stringify({ success: true, status: "analysis_initiated", bot_id }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    } catch (error) {
  ...
  return new Response(
    JSON.stringify({ status: "ignored", reason: "Status not relevant" }),
    { headers: { ...corsHeaders, "Content-Type": "application/json" } }
  );
}
...
async function handleTranscriptDone(data) {
  ...
  if (!botId) {
    ...
    return new Response(
      JSON.stringify({ error: "Missing bot ID in transcript.done event" }),
  ...
  if (!transcriptId) {
    ...
    return new Response(
      JSON.stringify({ error: "Missing transcript ID in transcript.done event" }),
      { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: true, status: "transcript_processing_started", botId, transcriptId }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
...
async function handleAnalysisDone(data) {
  ...
  if (!botId) {
    ...
    return new Response(
      JSON.stringify({ error: "Missing bot ID in analysis_done event" }),
      { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: true, status: "transcript_fetched", botId }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
  ...
  try {
    ...
    if (transcriptData && transcriptData.transcript) {
      if (typeof transcriptData.transcript === 'string') {
        ...
        if (Array.isArray(monologue) && monologue.length > 0 && 
            monologue[0].messages && monologue[0].messages.length > 0) {
    ...
    const { data: existingTranscript, error: findError } = await supabase
      .from('transcripts')
      .select('*')
      .eq('bot_id', botId)
    ...
    const transcriptRecord = {
      ...
      recall_transcript_id: transcriptId, // Add this line
    ...
    if (savedTranscriptId) {
      try {
        ...
        const { data: processResult, error: processError } = await supabase.functions.invoke('process-transcripts', {
          body: { transcriptId: savedTranscriptId }
        });
